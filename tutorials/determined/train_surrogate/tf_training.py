...
def train_model(context, model, epochs):
    ...
    for epoch in range(epochs):
        ...
        history = model.fit(training_generator, steps_per_epoch=None,
        ...
        context.train.report_validation_metrics(
            steps_completed=epoch+1,
            metrics={'loss': history.history['loss'][-1]}
        )
        if (epoch+1)%10 == 0:
            store_model(model, epoch//10)
            with context.checkpoint.store_path({"steps_completed": epoch+1}) as (path, uuid):
                model.save(
                    path,
...
if __name__ == "__main__":
    ...
    with det.core.init() as context:
        ...
        train_model(context, vae, args.epochs)
